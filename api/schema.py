...
class Query(graphene.ObjectType):
    all_universities = graphene.List(UniversityType,name = graphene.String(),
    country = graphene.String(),
    alpha_two_code = graphene.String(), domain=graphene.String())
    ...
    def resolve_all_universities(parent, info, **search):
        if search:
            ...
            if country and name:
                ...
                return University.objects.filter(name__icontains=name).filter(alpha_two_code__iexact=alpha_two_code)
            elif country:
            ...
            raise GraphQLError("your search did not return any results.")
